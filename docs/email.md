...
invites, but not delivery — sending is application-specific. The bundled
`asterion.extensions.email` extension provides ready-made delivery over SMTP or
a transactional-email provider, overridable templates, custom app events, and an
optional transactional outbox. Core ships only the notifier Protocols plus
dev-only logging defaults, so nothing here is pulled in unless you opt in.
...
One notifier instance satisfies **both** the `PasswordResetNotifier` and
`InviteNotifier` Protocols, so it goes into both `create_admin` keywords:
...
### SMTP environment variables (`from_env`)
...
| `ASTERION_SMTP_USERNAME` / `_PASSWORD` | — | Auth credentials. |
...
pages — e.g. `https://app.example.com/accept?token={token}` — so the emails
carry a real link instead of the raw token.

## Provider alternatives to SMTP
...
Both render identically (see [Templates](#templates)) and differ only in how the
message is sent (Resend → JSON HTTP API via httpx; SES → boto3 SESv2). For tests
or to route through your own pipeline, pass `send=` (Resend/SES) or `transport=`
(SMTP) — a callable that receives the rendered payload/message.

## Templates

Bodies render from Jinja templates named `<name>.subject.txt`, `<name>.txt`, and
(optionally) `<name>.html`. They resolve from your `template_dir` first, then
...
Template context: `app_name`, `email`, `token`, `link`, `has_url` (reset); plus
`tenant_slug` (invite). For full programmatic control, subclass and override
`render_reset` / `render_invite` instead. Without `jinja2` installed, the
notifier falls back to built-in plaintext.

## Custom app events

Beyond reset and invite, send your own emails (welcome, receipt, "export ready",
...
`<event>` Jinja template, else raises `KeyError`. Renderers can also be passed
up front via `SmtpEmailNotifier(templates={...})`.

## Robust delivery: the transactional outbox

Sending inline couples the request to the mail server: a slow SMTP call slows
the invite response, and a transient failure loses the email. Wrap any notifier
in `OutboxEmailNotifier` to **persist** the email in the *same* DB transaction
as the invite/user that triggered it, then send it from a background worker:
...
A worker drains the queue with bounded retries and backoff:
...
Run it from your own cron/loop (e.g. every minute). Outside a request (e.g. a
job that sends mail directly), give the outbox a session factory:
`OutboxEmailNotifier(real, session_factory=runtime.db.session)`.
...
* [Multi-tenancy § Invite delivery](tenancy.md#invite-delivery) — the invite
  token lifecycle this delivers.
* [Extensions](extensions.md) — the SPI and the migration convention.
